...
function saveShopState(state) {
  const safe = {
    ...
    bonusBirdRateLevel: Math.max(0, Math.min(4, Number(state?.bonusBirdRateLevel) || 0))
  };
  try {
...
class GameScene extends Phaser.Scene {
  ...
  handleBulletHitBird(bullet, bird) {
    ...
    if (isBoss) {
      ...
      const bossHit = this.add.image(bx, by, "hit").setScale(0.30).setTint(0xffb300).setDepth(18);
      this.tweens.add({ targets:bossHit, alpha:0, scale:0.60, duration:500, onComplete:()=>bossHit.destroy() });
      for(let i=0;i<12;i++){
    ...
    if(isDanger) {
      ...
      const hit = this.add.image(bx, by, this.getHitTexture()).setScale(0.18).setTint(0xff0000).setDepth(18);
      this.tweens.add({ targets:hit, alpha:0, scale:0.35, duration:400, onComplete:()=>hit.destroy() });
    ...
    const hit = this.add.image(bx, by, this.getHitTexture()).setScale(isCrit ? 0.22 : 0.17).setDepth(18);
    if (isCrit) hit.setTint(0xff8a65);
    this.tweens.add({ targets:hit, alpha:0, scale:isCrit ? 0.40 : 0.30, duration:350, onComplete:()=>hit.destroy() });
  ...
  applyBonusBird(bonusType, bx, by) {
    ...
    const hitImg = this.add.image(bx, by, "hit").setScale(0.19).setTint(bonusColors[bonusType]).setDepth(18);
    this.tweens.add({ targets:hitImg, alpha:0, scale:0.36, duration:400, onComplete:()=>hitImg.destroy() });
